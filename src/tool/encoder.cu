...
// 	if (tid < Nh * nresidues){
// 		// swap first half
// 		int new_cid1 = (cid * pow) % M;
// 		// swap second half
// 		int new_cid2 = ((cid + Nh) * pow) % M;
// 		// bit flips
// 		int bit_cid1 = (new_cid1 > N);
// 		int bit_cid2 = (new_cid2 > N);
// 		new_cid1 -= bit_cid1 * N;
// 		new_cid2 -= bit_cid2 * N;
		
// 		// 
// 		GaussianInteger x = a[cid + rid * Nh];
// 		uint64_t x1 = x.re;
// 		uint64_t x2 = x.imag;

// 		uint64_t new_x1 = (bit_cid1 ? RNSCoprimes[rid] - x1 : x1);
// 		uint64_t new_x2 = (bit_cid2 ? RNSCoprimes[rid] - x2 : x2);
// 		// 
// 		if(new_cid1 < Nh)
// 			b[new_cid1  + rid * Nh].re = new_x1;
// 		else
// 			b[(new_cid1 - Nh)  + rid * Nh].imag = new_x1;
// 		if(new_cid2 < Nh)
// 			b[new_cid2  + rid * Nh].re = new_x2;
// 		else
// 			b[(new_cid2 - Nh)  + rid * Nh].imag = new_x2;
...
__host__ void rotate_slots_right(
	...
	int N = (Nh << 1);
	int M = (N << 1);
	int nresidues = CUDAEngine::get_n_residues(base);
	
	// Pre-compute the map
	uint64_t rotGroup    = conv<long>(NTL::PowerMod(to_ZZ(5), rotSlots, to_ZZ(M)));
	uint64_t invRotGroup = conv<long>(NTL::InvMod(to_ZZ(rotGroup), to_ZZ(M)));
	...
	const dim3 gridDim(ADDGRIDXDIM);
	const dim3 blockDim(ADDBLOCKXDIM);
	cudaMemsetAsync(ctx->d_aux_coefs, 0, size * sizeof(GaussianInteger), ctx->get_stream());
  	cudaCheckError();
	...
	cudaMemcpyAsync(d_a, ctx->d_aux_coefs, size * sizeof(GaussianInteger), cudaMemcpyDeviceToDevice, ctx->get_stream());
  	cudaCheckError();
