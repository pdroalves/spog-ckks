...
__host__ Keys* ckks_keygen(CKKSContext *ctx, SecretKey *sk){
	...
	for (int i = 1; i < CUDAEngine::N; i *= 2){
		...
		rotate_slots_right(ctx, s2.d_coefs, QBBase, i);
	...
	for (int i = 1; i < CUDAEngine::N; i *= 2){
		poly_copy(ctx, &s2, &sk->s);
		rotate_slots_left(ctx, s2.d_coefs, QBBase, i);
	...
	// Conjugation key //
	poly_copy(ctx, &s2, &sk->s);
	conjugate_slots(ctx, s2.d_coefs, QBase);
...
__host__ void ckks_mul_without_rescale(
	...
	// ModUp
	poly_approx_basis_raising_Q_to_QB(ctx, ctx->d2axaxQB, &ctx->axax, c3->level);
	...
	// // ModDown
	poly_approx_basis_reduction_QB_to_Q(ctx, &ctx->d_tildeQ[0], &ctx->d_tildeQB[0], c3->level);
	poly_approx_basis_reduction_QB_to_Q(ctx, &ctx->d_tildeQ[1], &ctx->d_tildeQB[1], c3->level);
...
__host__ void ckks_square_without_rescale(
	...
	// ModUp
	poly_approx_basis_raising_Q_to_QB(ctx, ctx->d2axaxQB, &ctx->axax, c1->level);
	...
	// // ModDown
	poly_approx_basis_reduction_QB_to_Q(ctx, &ctx->d_tildeQ[0], &ctx->d_tildeQB[0], c1->level);
	poly_approx_basis_reduction_QB_to_Q(ctx, &ctx->d_tildeQ[1], &ctx->d_tildeQB[1], c1->level);
...
__host__ cipher_t* ckks_add_const(
	CKKSContext *ctx,
	cipher_t *c1,
	double val){
	assert(CUDAEngine::is_init);
	...
	ckks_add_const(ctx, c2, c1, val);

	return c2;
}

__host__ void ckks_mul_const(
	CKKSContext *ctx,
	cipher_t *c2,
	cipher_t *c1,
	double val){
...
__host__ cipher_t* ckks_mul_const(
	CKKSContext *ctx,
	cipher_t *c1,
	double val){
	assert(CUDAEngine::is_init);
...
__host__ void ckks_eval_polynomial(
	...
	for(int i = n-1; i >= 0; i--){
		...
		ckks_add_const(ctx, result, result, coeffs[i]);
	}
...
__host__ void ckks_log1minus(
	...
	ckks_add_const(ctx, result, result, -1);
	ckks_mul_const(ctx, result, result, -1);
...
__host__ void ckks_rotate_right(
	...
	rotate_slots_right(ctx, c2->c[0].d_coefs, c2->c[0].base, rotSlots);
	rotate_slots_right(ctx, ctx->axax.d_coefs, ctx->axax.base, rotSlots);
	...
	// // ModDown
	poly_approx_basis_reduction_QB_to_Q(ctx, &ctx->d_tildeQ[0], &ctx->d_tildeQB[0], c1->level);
	poly_approx_basis_reduction_QB_to_Q(ctx, &c2->c[1], 		&ctx->d_tildeQB[1], c1->level);
...
__host__ void ckks_rotate_left(
	...
	rotate_slots_left(ctx, c2->c[0].d_coefs, c2->c[0].base, rotSlots);
	rotate_slots_left(ctx, ctx->axax.d_coefs, ctx->axax.base, rotSlots);
...
__host__ void ckks_conjugate(
	...
	conjugate_slots(ctx, c2->c[0].d_coefs, c2->c[0].base);
	conjugate_slots(ctx, ctx->axax.d_coefs, ctx->axax.base);
...
__host__ void ckks_inverse(
	...
	ckks_add_const(ctx, &ctx->aux[0], ct, -1); // cbar
	ckks_mul_const(ctx, &ctx->aux[0], &ctx->aux[0], -1);

	ckks_add_const(ctx, result, &ctx->aux[0], 1); // tmp

	for (int i = 1; i < steps; i++){
		ckks_square(ctx, &ctx->aux[0], &ctx->aux[0]);
		ckks_add_const(ctx, &ctx->aux[1], &ctx->aux[0], 1);
...
// 	ckks_add_const(ctx, &ctx->aux[3], ct, -1);
// 	ckks_add_const(ctx, &ctx->aux[4], ct, 1);
...
__host__ void ckks_log(
	...
	// Compute the new evaluation point
	ckks_add_const(ctx, &ctx->aux[0], ct, -1);
	...
	for(int i = 0; i < steps; i++){
		ckks_mul_const(ctx, &ctx->aux[2], &ctx->aux[1], 1.0 / (i+1));
		if(i % 2 == 0)
			ckks_add(ctx, result, result, &ctx->aux[2]);
		else
