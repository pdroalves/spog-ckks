...
__host__ void cipher_init(Context *ctx, cipher_t *c, int slots, int base){
...
json cipher_export(Context *ctx, cipher_t *ct){
	std::string ct0 = poly_export_residues(ctx, &ct->c[0]);
	std::string ct1 = poly_export_residues(ctx, &ct->c[1]);
...
cipher_t* cipher_import(Context *ctx, const json & k){
	...
	poly_copy(ctx, &ct->c[0], poly_import_residues(ctx, k["ct"]["0"].GetString()));
	poly_copy(ctx, &ct->c[1], poly_import_residues(ctx, k["ct"]["1"].GetString()));
