...
TEST_P(TestCKKSBasicHomomorphism, AddByConstant)
{
	...
	for(int N = 0; N < NTESTSINTENSE; N++){
		...
		// Mul //
		/////////
		cipher_t *ct2 = ckks_add_const(cipher, ct1, val2);
...
TEST_P(TestCKKSBasicHomomorphism, MulByConstant)
{
	...
	for(int N = 0; N < NTESTSINTENSE; N++){
		...
		// Mul //
		/////////
		cipher_t *ct2 = ckks_mul_const(cipher, ct1, val2);
...
TEST_P(TestCKKSBasicHomomorphism, DivByConstant)
{
	...
	for(int N = 0; N < NTESTSINTENSE; N++){
		...
		// Mul //
		/////////
		cipher_t *ct2 = ckks_mul_const(cipher, ct1, 1.0 / val2);
...
INSTANTIATE_TEST_CASE_P(SPOGCKKS,
	TestCKKSBasics,
	basicparams,
	printParamName
);
...
int main(int argc, char **argv) {
  ...
  std::cout << "Testing SPOGCKKS " << GET_SPOGCKKS_VERSION() << std::endl;
